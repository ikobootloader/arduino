...
{//Créer un intervalle de temps à partir de la fonction minuterie()
  if(freq2 == 1)
    ntvrl++;
}
  ...
  if(memoParcoursBool == true){ //Si détection du parcours  
     ...
     */  
     ...
     if(parcoursEtape >= 10 && parcoursEtape <= 19){
      ...
      //si minuterie à 250 millisecondes > couperNombre à multiplier par 4 ? > en test !
      tempoParcours = minuterie(250); //tempoParcours est à true > on change d'étape
      ...
      //le passage à la fonction score() suffit à changer l'état de la fonction memoParcours() en false
